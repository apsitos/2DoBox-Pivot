...
/***/ function(module, exports, __webpack_require__) {
	...
	$('.idea-list').on('click', '.completed', function () {
	  ...
	  var thisIdea = $(this).parent().prop('id');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _dobox = __webpack_require__(1);
	...
	$('ul').on('click', '.up-vote', function () {
	  ...
	  var id = this.closest('li').id;
	  for (var i = 0; i < _dobox.ideaArray.length; i++) {
	    if (_dobox.ideaArray[i].id == id) {
	      _dobox.ideaArray[i].quality = newQuality;
	      _dobox.ideaArray.splice(i, 1, _dobox.ideaArray[i]);
	    }
	  }
	  $(this).closest("li").find(".user-quality").text(newQuality);
	  (0, _dobox.saveToStorage)();
	});
